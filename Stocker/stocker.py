...
class Stocker():
    ...
    def __init__(self, price):
        ...
        self.starting_price = float(self.stock.iloc[0, 'Adj. Open'])
        
        # The most recent price
        self.most_recent_price = float(self.stock.iloc[len(self.stock) - 1, 'y'])
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Adj. Close'], plot_type='basic'):
        ...
        for i, stat in enumerate(stats):
            ...
            print('Current {} = {:.2f} on {}.\n'.format(stat, self.stock.iloc[len(self.stock) - 1, stat], self.max_date.date()))
    ...
    def create_prophet_model(self, days=0, resample=False):
        ...
        if days > 0:
            ...
            print('Predicted Price on {} = ${:.2f}'.format(
                future.iloc[len(future) - 1, 'ds'], future.iloc[len(future) - 1, 'yhat']))
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        if not nshares:
            ...
            print('\nPredicted price on {} = ${:.2f}.'.format(max(future['ds']), future.iloc[len(future) - 1, 'yhat']))
            print('Actual price on    {} = ${:.2f}.\n'.format(max(test['ds']), test.iloc[len(test) - 1, 'y']))
        ...
        # If a number of shares is specified, play the game
            ...
            for i, correct in enumerate(test_pred_increase['correct']):
                ...
                if correct == 1:
                    prediction_profit.append(nshares * test_pred_increase.iloc[i, 'real_diff'])
            ...
            test['hold_profit'] = nshares * (test['y'] - float(test.iloc[0, 'y']))
            ...
            final_profit = test.iloc[len(test) - 1, 'pred_profit']
            final_smart = test.iloc[len(test) - 1, 'hold_profit']

            # text location
            last_date = test.iloc[len(test) - 1, 'ds']
    ...
    def changepoint_date_analysis(self, search=None):
        ...
        if not search:
            ...
            print(c_data.iloc[:, ['Date', 'Adj. Close', 'delta']][:5])
    ...
    def changepoint_prior_validation(self, start_date=None, end_date=None,changepoint_priors = [0.001, 0.05, 0.1, 0.2]):
        ...
        for i, prior in enumerate(changepoint_priors):
            ...
            results.iloc[i, 'train_err'] = avg_train_error
            results.iloc[i, 'train_range'] = avg_train_uncertainty
            ...
            results.iloc[i, 'test_err'] = avg_test_error
            results.iloc[i, 'test_range'] = avg_test_uncertainty
